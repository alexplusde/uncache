...
if (rex_addon::get('ycom')->isAvailable()) {
    echo rex_view::info($this->i18n('uncache_info_ycom_active'));
}
if (rex_addon::get('url')->isAvailable()) {
    echo rex_view::info($this->i18n('uncache_info_url_active'));
}

if (!rex_request::isHttps() && $_SERVER['SERVER_PROTOCOL'] != "HTTP/2.0") {
    echo rex_view::error($this->i18n('uncache_no_http2') . $_SERVER['SERVER_PROTOCOL']);
}

echo rex_view::info($this->i18n('uncache_info_ep'));

if (!rex_addon::get('media_manager_responsive')->isAvailable()) {
    $anchor = '<a target="_blank" href="https://github.com/alexplusde/media_manager_responsive/">'.$this->i18n('uncache_info_media_manager_responsive_install').'</a>';
    $fragment = new rex_fragment();
    $fragment->setVar('class', 'info', false);
    $fragment->setVar('title', $this->i18n('uncache_info_media_manager_responsive_title'), false);
    $fragment->setVar('body', '<p>' . $this->i18n('uncache_info_media_manager_responsive_inactive') . '</p>' . $anchor, false);
